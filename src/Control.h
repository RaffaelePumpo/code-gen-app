...
class Control final
{
 ...
 public:
  ...
  struct B_Control_T {
    real_T position;                   // '<Root>/Sensing'
    real_T velocity;                   // '<Root>/Sensing'
  };
  ...
  // model step function
  void Control_Sensingdecode(const real_T *rtu_State_position, const real_T
    *rtu_State_velocity);

  // model step function
  void Control_LQRcontrol(real_T *rty_Torque);
 ...
 private:
  ...
  // model instance variable for '<Root>/LQR'
  LQR LQRMDLOBJ1;

  // model instance variable for '<Root>/Sensing'
  Sensing SensingMDLOBJ2;
